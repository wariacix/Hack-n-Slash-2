...
std::wstring equipmentSystem::Equipment::loadItemStats()
{
	std::string filename("C:\\Users\\waria\\source\\repos\\Hack-n-Slash-2\\Generator\\itemStats.txt");
...
{
	std::wstring load = loadItemStats();
	std::wstring stats[15];
				...
				for (int u = 0; u < numOfItems; u++)
				{
					...
					else if (u == numOfItems - 1 and addedItem == false)
					{
						setItem(Item(std::stoi(stats[0]), std::stoi(stats[1]), std::stoi(stats[2]), std::stoi(stats[3]),
									 ...
									 std::stoi(stats[8]), std::stoi(stats[9]), std::stoi(stats[10]), howManyItems,
									 std::stoi(stats[11]), std::stoi(stats[12]), std::stoi(stats[13]), stats[14]));
						addNumOfItems(1);
						addedItem = true;
					}
				}
